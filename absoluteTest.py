...
class nchain:
    ...
    def DO(self,action):
        if action == 0: #buy
            self.reward = np.abs( (self.state+1)  - 10) - np.abs(self.state-10)
        elif action == 1: #sell
            self.reward = np.abs(self.state - 10) - np.abs( (self.state+1)  - 10)
        else:
            print('error')
        self.state += 1
        if self.state == 21:
            self.done = True
        # remember=(np.abs(self.state - 11))
...
def q_learning_keras(env, num_episodes=1000):
    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            target_vec[a] = target
            model.fit(np.identity(22)[s:s + 1], target_vec.reshape(-1, 2), epochs=1, verbose=0)
            s = new_s
            r_sum += r
